...
#### 表記ゆれをAIで統一
右上の「✨ 表記ゆれをAIで統一」ボタンを押すと、**現在選択中の AI プロバイダー**が食材名の表記ゆれ(例: 「鶏もも」と「鶏もも肉」、「玉ねぎ」と「たまねぎ」)を検出します。

- 検出された統一案(`別表記 → 正規名`)が一覧で表示され、確認のうえ実行できます
- 実行すると食材マスタ(`database.csv`)から重複する別表記が削除され、**その食材を使っている全レシピも正規名へ自動で書き換え**られます
- レシピ内で重複した食材になった場合は1つにまとめられます
- 本当に別の食材(「豚もも」と「鶏もも」など)は統合されません
...
ちなみに自分のM2MacBook(8GB)は2Bモデルで撃沈、デスクトップ(メモリー32GB+RTX4060)は4Bモデルまでいけました。
