...
確認済みでないプロバイダも一般的なテキスト生成AIであれば動作します。\
...
### 食材管理
登録済みの食材を一覧表示し、カテゴリーの変更や並び順の入れ替え、`database.csv` への保存ができます。
...
### レシピ管理
![RMP_Manage_Ingredient.png](Photo/RMP_Manage_Ingredient.png)
各食材のカテゴリー変更と表示する順番の変更を行えます。\
レシピの登録の食材は上から読み取るため、頻繁に使う食材を上にしておくことをおすすめします。\
csvファイルを直接編集しても同じように反映されます。

#### 表記ゆれをAIで統一
右上の「表記ゆれをAIで統一」ボタンを押すとAIが食材名の表記ゆれ(例: 「鶏もも」と「鶏もも肉」、「玉ねぎ」と「たまねぎ」)を検出します。

- 検出された統一案(`別表記 → 正規名`)が一覧で表示され、確認のうえ実行できます。
- 実行すると食材マスタ(`database.csv`)から重複する別表記が削除され、その食材を使っている全レシピも正規名へ自動で書き換えられます。
- レシピ内で重複した食材になった場合は1つにまとめられます。
...
URLバッチ取り込みはtxtファイルに入力したURLを全て参照し、AIが自動的にレシピに登録するものです(カテゴリーは自動的にその他になります)
...
ちなみに自分のM2MacBook(8GB)は2Bモデルで撃沈、デスクトップ(メモリー32GB+RTX4060)は4Bモデルは余裕で8Bは時間がかかる感じでした。\
ローカルAIを使うならVRAM16GB以上は欲しいけど、最近高すぎてアップグレード前提で買ったPCそのまま放置。
...
なおWhisperよりGeminiのほうが精度が高いため、 **Geminiの利用を推奨します** 。

### 設計について
PlantUMLでUML.pumlを開くと出てきます。\
たまに加えたクラスをUMLには加え忘れることもあるから、そのときはm(_ _)m ｺﾞﾒﾝ
...
- AndroidとかiOSとかで動かしたい\
  ~iOSはJavaに対応してくれまじで頼むDartかSwiftでこれもう一つ作るん嫌すぎる勘弁してくれ~
